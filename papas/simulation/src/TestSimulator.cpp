...
TestSimulator::TestSimulator(const Particles& particles, const Detector& detector, Clusters& ecalClusters,
...
void TestSimulator::smearMuon(SimParticle& ptc) {
  ...
  void TestSimulator::simulateMuon(SimParticle& ptc) {
...
Cluster TestSimulator::makeCluster(const SimParticle& ptc, papas::Layer layer, double fraction, double csize,
                                   char subtype) const {
...
bool TestSimulator::acceptElectronSmearedTrack(const Track& smearedTrack, bool accept) const {
  ...
  if (m_detector.electronAcceptance(smearedTrack) || accept) {
    ...
    return false;
      ...
      bool TestSimulator::acceptMuonSmearedTrack(const Track& smearedTrack, bool accept) const {
        // decide whether the electron smearedTrack is detected
...
std::shared_ptr<const DetectorElement> TestSimulator::elem(papas::Layer layer) const {
