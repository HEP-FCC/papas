...
class PFReconstructor {
   ...
   * The PFevent may (in theory) be constructed from clusters and tracks read in from file or may be simulated in PAPAS
   ...
   - if excess = hcal energy + ecal energies - track energies > 0 and excess < ecal energies then turn the excess into
   an
   photon
...
private:
  ...
  */
  ...
  @param parentIds ids of parent objects which will be recorded in the history
  @param energy Energy that is to be assigned to the particle, if not specified or if negative the cluster energy will
  be used
  ...
  */
  ...
  /**  Checks if object identifier comes, directly or indirectly,
   ...
   @return True if there is a direct /indirect link between object and particle of this typeAndSubtype and absolute
   pdgid
  */
