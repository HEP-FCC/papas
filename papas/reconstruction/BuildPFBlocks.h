...
namespace papas {
  /**buildPFBlocks
   * This takes a list of Ids and an unordered map of associated edges which have distance and link info
  ...
  /**
   This function takes collections of tracks and clusters from an event and calculates the
   distances and links (edges) between the elements. These are then used to construct subgraphs
   and the PFBlocks from the subgraphs
   ...
   * @param[inout] history : external collection of Nodes to which parent child relations can be added
   */
  void buildPFBlocks(const Event& event, const std::string& ecalTypeAndSubtype,
