...
namespace papas {
   ...
   *  @brief The PapasManager class is a high level class to own the clusters, particles etc created in an event and 
   ...
   lightweight class containing unordered_maps of pointers to const collections that can be used along side an object id to retreive a specific object.
   The choice to make the PapasManager own the cluster collections, and to make the PapasEvent store pointers to const collections means that the Papas Algorithms and the PapasEvent object can be called as Gaudi Algorithms within the
     FCCSW framework.
  ...
  private:
    ...
    SimParticles& createParticles(); //Create an empty concrete collection of particles ready for filling by an algorithm
