...
#ifndef papas_logger_h
#define papas_logger_h
...
///      macro is used and it must return a papaslog::Logger object.
///
/// The debug message is printed if the current papaslog::Logging::Level <=
/// papaslog::Logging::VERBOSE.
#define PAPASLOG_VERBOSE(x)                                                        \
if (papaslog::logger()->doPrint(papaslog::Logging::VERBOSE))                                \
papaslog::logger()->log(papaslog::Logging::VERBOSE) << x;
...
///
/// The debug message is printed if the current papaslog::Logging::Level <=
/// papaslog::Logging::DEBUG.
#define PAPASLOG_DEBUG(x)                                                          \
if (papaslog::logger()->doPrint(papaslog::Logging::DEBUG))                                  \
papaslog::logger()->log(papaslog::Logging::DEBUG) << x;
...
///
/// The debug message is printed if the current papaslog::Logging::Level <=
/// papaslog::Logging::INFO.
#define PAPASLOG_INFO(x)                                                           \
if (papaslog::logger()->doPrint(papaslog::Logging::INFO))                                   \
papaslog::logger()->log(papaslog::Logging::INFO) << x;
...
///
/// The debug message is printed if the current papaslog::Logging::Level <=
/// papaslog::Logging::WARNING.
#define PAPASLOG_WARNING(x)                                                        \
if (papaslog::logger()->doPrint(papaslog::Logging::WARNING))                                \
papaslog::logger()->log(papaslog::Logging::WARNING) << x;
...
///
/// The debug message is printed if the current papaslog::Logging::Level <=
/// papaslog::Logging::ERROR.
#define PAPASLOG_ERROR(x)                                                          \
if (papaslog::logger()->doPrint(papaslog::Logging::ERROR))                                  \
papaslog::logger()->log(papaslog::Logging::ERROR) << x;
...
///
/// The debug message is printed if the current papaslog::Logging::Level <=
/// papaslog::Logging::FATAL.
#define PAPASLOG_FATAL(x)                                                          \
if (papaslog::logger()->doPrint(papaslog::Logging::FATAL))                                  \
papaslog::logger()->log(papaslog::Logging::FATAL) << x;
// clang-format on
...
namespace papaslog {
  ...
  namespace Logging {
    ...
    public:
      ...
      OutStream(const OutStream& copy) : m_stream(), m_outputFunctor(copy.m_outputFunctor) {
    ...
    public:
      ...
      ///        decorator object
      explicit OutputDecorator(std::shared_ptr<OutputPrintPolicy> wrappee) : m_wrappee(std::move(wrappee)) {}
      ...
      /// object.
      void flush(const Level& lvl, const std::ostringstream& input) override { m_wrappee->flush(lvl, input); }
      
    private:
      /// wrapped object for printing the debug message
      std::shared_ptr<OutputPrintPolicy> m_wrappee;
    };
    ...
    public:
      ...
      NamedOutputDecorator(std::shared_ptr<OutputPrintPolicy> wrappee, const std::string& name, unsigned int maxWidth = 15)
    ...
    public:
      ...
      TimedOutputDecorator(std::shared_ptr<OutputPrintPolicy> wrappee, const std::string& format = "%X")
    ...
    public:
      ...
      ///
      /// @param [in] wrappee output print policy object to be wrapped
      explicit ThreadOutputDecorator(std::shared_ptr<OutputPrintPolicy> wrappee) : OutputDecorator(std::move(wrappee)) {}
    ...
    public:
      ...
      ///
      /// @param [in] wrappee output print policy object to be wrapped
      explicit LevelOutputDecorator(std::shared_ptr<OutputPrintPolicy> wrappee) : OutputDecorator(std::move(wrappee)) {}
    ...
    public:
      ...
      /// @param [in] lvl   debug level of debug message
      /// @param [in] input text of debug message
      void flush(const Level& lvl, const std::ostringstream& input) final { (*m_out) << input.str() << std::endl; }
  ...
  public:
    ...
    Logger(std::shared_ptr<Logging::OutputPrintPolicy> pPrint, std::shared_ptr<Logging::OutputFilterPolicy> pFilter)
    ...
    bool doPrint(const Logging::Level& lvl) const { return m_filterPolicy->doPrint(lvl); }
    ...
    Logging::OutStream log(const Logging::Level& lvl) const {
      return Logging::OutStream(
                                std::bind(&Logging::OutputPrintPolicy::flush, m_printPolicy.get(), lvl, std::placeholders::_1));
    }
    
  private:
    /// policy object for printing debug messages
    std::shared_ptr<Logging::OutputPrintPolicy> m_printPolicy;
    
    /// policy object for filtering debug messages
    std::shared_ptr<Logging::OutputFilterPolicy> m_filterPolicy;
  };
  ...
  getDefaultLogger(const std::string& name, const Logging::Level& lvl, std::ostream* log_stream = &std::cout);

  //global main log for papas
  extern std::shared_ptr<const papaslog::Logger> papaslogger;
  extern std::shared_ptr<const papaslog::Logger> logger();

  
}  // end of namespace papaslog

#endif /* papas_logger_h */
