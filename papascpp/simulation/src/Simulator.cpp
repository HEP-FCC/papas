...
Simulator::Simulator(const Detector& d, Nodes & nodes) : m_detector(d),  m_nodes(nodes), m_propStraight(),
...
void Simulator::SimulateParticle(const Particle& ptc, IdType parentid ) {
  ...
  else if (abs(pdgid) >= 100) {  // TODO make sure this is ok
  ...
  SimParticle& Simulator::addGunParticle(unsigned int pdgid,
                                         double charge,
                                         double thetamin,
                                         double thetamax,
                                         double ptmin,
  ...
  SimParticle& Simulator::addParticle(
      unsigned int pdgid, double charge, double theta, double phi, double energy, TVector3 vertex) {
    double mass = ParticlePData::particleMass(pdgid);
  ...
  Id::Type Simulator::addSmearedCluster(
      Id::Type parentClusterId, papas::Layer detlayer, papas::Layer acceptlayer, bool accept)
