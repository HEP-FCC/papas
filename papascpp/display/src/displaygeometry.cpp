...
GDetectorElement::GDetectorElement(std::shared_ptr<const DetectorElement> detElem)
{   //For each inner and outer cyclinder create a cyclinder (circular cross section) for xy , yz, xz projections
   ...
   for (auto const& elem : {detElem->volumeCylinder().outer(), detElem->volumeCylinder().inner()}) { //AJRTODO should be if inner ......
