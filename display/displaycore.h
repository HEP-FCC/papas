...
#include "../utility/enummanager.h"

class ViewPane;
class Drawable;

ENUM_WITH_STRINGS(enumProjection,xy,yz,xz,ECAL_thetaphi,HCAL_thetaphi)
 ...
 *ENUM_WITH_STRING(enumProjection,(xy,yz,xz,ECAL_thetaphi,HCAL_thetaphi));
 */
...
public:
   ...
   Display(std::list<enumProjection> views = {}) ;
   //It contains several viewpanes each of which is a different projection
   //change to capitals
   ///Which things to draw (and which are locked - clearable=false)
...
public:
   ...
   ViewPane(enumProjection p, int nx, double xmin,
