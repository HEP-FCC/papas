...
int Id::uniqueId(long id) { return (id >> 16); }
...
long Id::makeTrackId(papas::SubType subtype) {
  return Id::makeId(DataType::kTrack, papas::Layer::kTracker, subtype, papas::enumSource::SIMULATION);
}

long Id::makeBlockId() {
  return Id::makeId(DataType::kBlock, papas::Layer::kNone, papas::SubType::kNONE, papas::enumSource::RECONSTRUCTION);
}

long Id::makeParticleId(eSource source) {
  return Id::makeId(DataType::kParticle, papas::Layer::kNone, papas::SubType::RAW, source);
}

bool Id::isCluster(long id) { return (isEcal(id) || isHcal(id)); }

bool Id::isEcal(long id) { return (Id::layer(id) == papas::Layer::kEcal); }

bool Id::isHcal(long id) { return (Id::layer(id) == papas::Layer::kHcal); }

bool Id::isTrack(long id) { return (Id::dataType(id) == DataType::kTrack); }

bool Id::isBlock(long id) { return (Id::dataType(id) == DataType::kBlock); }

bool Id::isUniqueIdMatch(long id, DataType datatype, papas::Layer layer, papas::SubType subtype) {
  return (Id::layer(id) == layer && Id::subType(id) == subtype && Id::dataType(id) == datatype);
}

bool Id::isUniqueIdMatch(long id, DataType datatype, papas::Layer layer, papas::SubType subtype,
                         papas::enumSource source) {
  return (Id::layer(id) == layer && Id::subType(id) == subtype && Id::dataType(id) == datatype &&
          Id::source(id) == source);
}

bool Id::isSmeared(long id) { return (Id::subType(id) == papas::SubType::SMEARED); }
