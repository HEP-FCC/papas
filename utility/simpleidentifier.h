...
public:
   ...
   static long makeIdentifier(fastsim::enumPFObjectType type, //chack name with Colin
                              eLayer layer,
                              fastsim::enumSubtype subtype,
                              fastsim::enumSource source,
                              int uniqueid = s_counter );
   static long makeAnotherIdentifier(long existingid);///uses same enums as existing id  but sets a new uniqueID.
