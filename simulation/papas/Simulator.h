...
public:
  ...
  Cluster smearCluster(const Cluster& cluster, papas::Layer detectorLayer = papas::Layer::kNone); ///<randomise cluster energy
...
private:
  ...
  const Track& storeSmearedTrack(Track&& smearedtrack, IdType parentid); ///<move into the smearedtracks collection and history
