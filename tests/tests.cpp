...
#include "datatypes/papas/Helix.h"
...
TEST(fastsim, Helix) {  /// Helix path test
...
TEST(fastsim, StraightLine) {
  ...
  PFParticle photon = PFParticle(uid, 22, 0, tlv);
  ...
  photon = PFParticle(uid, 22, 0, tlv);
  propStraight.propagateOne(photon, cyl1);
  propStraight.propagateOne(photon, cyl2);
  ...
  tlv = TLorentzVector(1, 0, 1, 2.);
  photon = PFParticle(uid, 22, 0, tlv, {0, 0, 1.5}, 0.);
  propStraight.propagateOne(photon, cyl1);
  points = photon.path()->points();
