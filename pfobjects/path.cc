...
Path::Path(const TLorentzVector& p4, const TVector3 & origin) :
   m_udir(p4.Vect().Unit()),
   m_speed(p4.Beta() * 299792458.0),
   m_origin(origin.X(),origin.Y(),origin.Z())
{
   //TODO AJR m_points["vertex"] = m_origin;
...
Helix::Helix( double field,double charge,const TLorentzVector&  p4,const TVector3 & origin) :
   Path(p4, origin),
...
double Helix::getPathLength(double deltat)  const{
   ...
   std::cout << m_omega <<" rho " << m_rho<< " vz " << getVZ() <<" deltat " <<deltat;
